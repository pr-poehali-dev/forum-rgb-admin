...
export default function ForumNew({ go }: Props) {
  ...
  const handleSubmit = () => {
    ...
    if (!titleCheck.ok) { setTitleError(titleCheck.reason || ''); hasError = true; }
    else if (title.trim().length < 5) { setTitleError('Заголовок слишком короткий'); hasError = true; }
    else setTitleError('');

    const bodyCheck = moderateText(body);
    if (!bodyCheck.ok) { setBodyError(bodyCheck.reason || ''); hasError = true; }
    ...
    go({ name: 'category', id: cat.id, title: cat.title, emoji: cat.emoji });
  };
    ...
    <div className="animate-fade-in">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="gradient-border rounded-3xl p-6">
          <h1 className="font-montserrat font-900 text-2xl gradient-text mb-6">Создать тему</h1>
          ...
          <div className="flex items-start gap-3 bg-purple-500/10 border border-purple-500/20 rounded-2xl p-4 mb-6">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground mt-1">
                Спам, реклама и запрещённые слова блокируются автоматически до публикации
              </div>
            </div>
          </div>
          ...
          <div className="mb-5">
            ...
            <div className="grid grid-cols-2 gap-2">
              {CATEGORIES.map(cat => (
                <button
                  ...
                  className={`flex items-center gap-3 p-3 rounded-xl border text-sm text-left transition-all ${category === cat.id
            ...
            {!category && shake && (
              <p className="text-xs text-red-400 mt-2">Выберите раздел</p>
            )}
          </div>
          ...
          <div className="mb-5">
            ...
            <input
              ...
              className={`w-full bg-white/5 border rounded-xl px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground transition-all ${shake && titleError ? 'animate-shake border-red-500/60' : 'border-white/10 focus:border-purple-500/60'}`}
            />
            {titleError && (
              <div className="flex items-center gap-2 text-xs text-red-400 mt-2">
                <Icon name="AlertCircle" size={13} />{titleError}
              </div>
            )}
          </div>
          ...
          <div className="mb-6">
            ...
            <textarea
              ...
              rows={6}
              className={`w-full bg-white/5 border rounded-xl px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground resize-none transition-all ${shake && bodyError ? 'animate-shake border-red-500/60' : 'border-white/10 focus:border-purple-500/60'}`}
            />
            {bodyError && (
              <div className="flex items-center gap-2 text-xs text-red-400 mt-2">
                <Icon name="AlertCircle" size={13} />{bodyError}
              </div>
            )}
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-gradient-to-r from-purple-500 to-pink-500 text-white text-sm font-600 hover:opacity-90 transition-opacity glow-purple"
            >
